...
#include "all_structure_forDy3.cuh"
...
__global__ void updateParent(int numS, xEdge_cuda* allChange_cuda, RT_Vertex* SSSP, int* changeFlag1, int* changeFlag2)
{
	...
	{
		if (allChange_cuda[index].done == 1)
		{
			printf("skip for edge ( %d, %d)\n", allChange_cuda[index].node1, allChange_cuda[index].node2);
			continue;
		}
		...
		if (SSSP[node_2].Dist > SSSP[node_1].Dist + edgeWeight)
		{
			if (SSSP[node_1].doneFlag > -1)
			{
				//allChange_cuda[index].done = 1;
				SSSP[node_2].doneFlag = index;
				//printf("inside if. node: %d, flag:%d", node_2, SSSP[node_2].doneFlag);
			}
			/*printf("doneflag 0 for e(%d,%d)", allChange_cuda[SSSP[node_2].doneFlag].node1, allChange_cuda[SSSP[node_2].doneFlag].node2);
			allChange_cuda[index].done = 1;*/
			...
			*changeFlag1 = 1; //every time node dist is updated, the flag becomes 1
			...
			//printf("*changeFlag1: %d", *changeFlag1);
			//*changeFlag2 = 1;
			printf("In updateParent if part:::: node:%d parent:%d edgewt:%f dist:%f done flag:e(%d,%d) \n", node_2, SSSP[node_2].Parent, edgeWeight, SSSP[node_2].Dist, node_1, node_2);
		//}
		//else {
		//	*changeFlag2 = 1;
		//	SSSP[node_2].Parent = node_1;
		//	SSSP[node_2].EDGwt = edgeWeight;
		//	//printf("In updateParent else part:::: node:%d parent:%d edgewt:%f dist:%f \n", node_2, SSSP[node_2].Parent, edgeWeight, SSSP[node_2].Dist);
		//}
		}
	}
}
...
__global__ void checkDist(int nodes, RT_Vertex* SSSP, int* changeFlag1, int* changeFlag2, xEdge_cuda* allChange_cuda)
{
	int idx = threadIdx.x + blockIdx.x * blockDim.x;
	int gridstride = gridDim.x * blockDim.x;
	...
	for (int index = idx; index < nodes; index += gridstride)
	{
		...
		double wt = SSSP[index].EDGwt;

		if (SSSP[index].doneFlag > -1)
		{
			allChange_cuda[SSSP[index].doneFlag].done = 1;
			parent = allChange_cuda[SSSP[index].doneFlag].node1;
			wt = allChange_cuda[SSSP[index].doneFlag].edge_wt;
			printf("\nnode:%d  parent:%d wt:%f", index, parent, wt);
		}

		//printf("\nnode:%d  parent:%d", index, parent);
		if (SSSP[index].Dist != SSSP[parent].Dist + wt)
		{
			SSSP[index].Dist = SSSP[parent].Dist + wt;
			printf("In checkDist:::: node:%d parent:%d edgewt:%f dist:%f\n", index, parent, SSSP[index].EDGwt, SSSP[index].Dist);
			//*changeFlag2 = 1;
			//*changeFlag1 = 1;
...
__global__ void helperFunction(int total_block, int thread_per_block, int nodes, int src, RT_Vertex* SSSP, double dummy_dist, int degree_frame, int totalChange, xEdge_cuda* allChange_cuda)
{
	if (threadIdx.x == 0)
	{
		...
		while (changeFlag1 == 1)
		{
			...
			//__syncthreads();
			cudaDeviceSynchronize();
			/*printf("after update parent\n");
			for (int i = 0; i < 12; i++)
			{

				printf("node:%d dist:%f parent:%d\n", i, SSSP[i].Dist, SSSP[i].Parent);
			}*/
			itr++;
			//if (changeFlag2 == 1)
			{
				//changeFlag2 = 0;
				checkDist << <total_block, thread_per_block >> > (nodes, SSSP, &changeFlag1, &changeFlag2, allChange_cuda);
				itr2++;
				//__syncthreads();
				cudaDeviceSynchronize();
				/*printf("After checkdist\n");
				for (int i = 0; i < 12; i++)
				{
