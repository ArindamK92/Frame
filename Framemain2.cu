...
__global__ void checkDist(int nodes, RT_Vertex* SSSP, int* change_d2)
{
	int idx = threadIdx.x + blockIdx.x * blockDim.x;
	int gridstride = gridDim.x * blockDim.x;
...
int main(int argc, char* argv[]) {
	...
	edge_update(&totalChange, allChange_cuda, SSSP, total_block, thread_per_block, &nodes, &degree_frame);
	//Time calculation
...
void edge_update(int* totalChange, xEdge_cuda* allChange_cuda, RT_Vertex* SSSP, int total_block, int thread_per_block, int* nodes, int* degree_frame)
{
	...
	change[0] = 1;
	int* change_d2 = new int[1];
	int* change2 = new int[1];
	change2[0] = 1;
	cudaMalloc((void**)&change_d, 1 * sizeof(int));
	cudaMalloc((void**)&change_d2, 1 * sizeof(int));
	int itr = 0, itr2 = 0;
	...
	{
		change2[0] = 0;
		cudaMemcpy(change_d2, change2, 1 * sizeof(int), cudaMemcpyHostToDevice);
		checkDist << <total_block, thread_per_block >> > (*nodes, SSSP, change_d2);
		//cudaDeviceSynchronize();
		cudaMemcpy(change2, change_d2, 1 * sizeof(int), cudaMemcpyDeviceToHost);
		itr2++;
	...
	while (change[0] == 1)
	{
		...
		itr++;
		if (change[0] == 1 /**&& itr % *degree_frame == 0**/)
		{
			change2[0] = 1;
		...
		cudaDeviceSynchronize();
		//printf("completed checkdist iteration: %d \n", itr2);
	...
	printf("No. of iteration for checkDist: %d \n", itr2);
	cudaFree(change_d);
	cudaFree(change_d2);
	return;
}
